...
- Palette placement from a non-desk surface pans to the FIRST desk with a status echo; a "most recently used desk" target may be friendlier — revisit with WP5's Map navigation. *(WP5: revisited — kept first-desk targeting everywhere, the map's Ctrl+D picker included; MRU-desk deferred to WP6 alongside its settings work.)*
...
**WP5 → WP6 handoffs:**
- Map deletions and link-rewires go stale until the next session's rebuild (`MapState::build`) — the live path only ADDS (`add_note` on created/external ids); deleted notes drop out of render (metas prefetch skips them) but keep their layout slot. Accepted for WP5 (live-add substitution for full ScanComplete deltas); revisit if it bites.
- A new note linked ONLY to orphans is classified an orphan and the edge is lost until rebuild (KNOWN GAP comment in `map.rs::add_note` — the orphan neighbor's later re-index hits the known-id guard, so nothing re-wires).
- Per-frame O(n) work while frozen (metas prefetch + orphan ring recompute over all nodes each frame) — fine at current scale, a scale-time concern for 20k-node vaults.
- Reduced-motion newcomers skip the FADE but still MOVE via the local reheat (interpretive reading of "appear settled") — decide whether reduced motion should pin newcomers at their seed instantly.
- Right-click context menu exists on the desk but not on the map or Drawer (Shift+F10/keyboard only there) — a three-way inconsistency for WP6's menus pass to unify.
- Ctrl+D-to-a-desk-the-card-is-already-on clobbers its position and journals a Place (drawer-inherited semantics), vs the palette's sacred-pan for already-placed cards — consider unifying in WP6.
- The corner mini panel may swallow hovers/clicks on map nodes beneath it (it allocates its rect last); and its content lags selection by one frame (selected_meta is prefetched before map_ui mutates focus).
- Rapid double `add_node` before the first newcomer resettles leaves the first newcomer's neighbors frozen where they stand — a second newcomer seeds against a cozier (unrelaxed) cluster. Cosmetic; noted from Task 1 review.
- (WP5x) Dark-mode palette input blends with the frame fill (`extreme_bg_color` == `card_plain_bg`) — pick a deliberate per-theme input color.
- (WP5x) Map canvas still renders `chrome_bg` while the desk paints `desk_bg` felt — paint the felt there too or update map.rs's dot-border contrast comment.
- (WP5x) Restyled zoom-button text sits on stock egui widget colors (untested pairs) — fold into the next selection-visuals pass.
- (WP5x) Rail section headers and palette row-hover/strata separators were scoped out of the polish pass.
- (WP5x) Mid-drag surface switch strands a live DragState until the desk next renders; selected-card edges point at the session position (not the live drag position) during a drag.
...
17. **Map layout mechanics (WP5, 2026-07-09):** Tunings frozen (jd-core `maplayout.rs`): `spring_k` 40, `repulsion` 800k, `damping` 0.85, `settle_eps` 0.5, rest length 120, repulsion grid cell 240 (2× rest, also the cutoff radius). Cooling is Fruchterman–Reingold-style but PER NODE: temperature 240·0.995^k, floored at `settle_eps`/2 — a per-node cap on displacement, so a local reheat cannot re-melt the field (the global bang-bang saturation the first cut had). `add_node` reheats the NEWCOMER ONLY to temperature 60 with the bulk — direct neighbors included — HARD-frozen (temp 0): measured bulk drift 0.009 px mean / 9.2 px max, resettle in 75 steps. Partial-cache rebuild (`MapState::build`): construct the layout from the CACHED subgraph only, freeze it with one zero-dt step, then `add_node` each uncached newcomer in sorted-id order — bulk stability is BITWISE (regression-tested); newcomer↔newcomer edges wire correctly because each pair's edge is inserted with the LATER endpoint, whose neighbor list already contains the earlier. Velocities are zeroed at freeze (no stored momentum to leak into a reheat). Initial placement of unpinned nodes is deterministic: jd-core xorshift seeded from the fixed constant `"JDMAP1"` (⊕ id bits), so identical inputs give identical layouts. Orphans (degree 0) are excluded from the physics entirely and rendered on a deterministic ring — radius = linked-cluster bounding radius + 200 px, bearing = splitmix-style hash of the NoteId — recomputed per frame from the live positions and never cached (the ring is a pure function of the cluster). Camera zoom-to-fits on map entry each session and is NOT persisted (positions are the stable thing). Per-frame step budget while unsettled: ≤ 4 fixed-dt (1/60 s) steps or 8 ms wall clock, whichever first; one 20k-node/40k-edge step is < 16 ms in the release-gated bench.

18. **Dogfood-driven visual pass (WP5x, 2026-07-10):** Scraps are 240×88 torn strips (was 240×130) with deterministic tears on BOTH top and bottom edges (±3 px jitter about 3 px-inset baselines, straight sides); their faces clip to WHOLE text rows (no mid-glyph cut). The Natural red rule sits BELOW the title (`RED_RULE_Y` 39.5) and rules align to the body row pitch (`RULE_SPACING` 18 — rules underline lines instead of striking through glyphs). Chrome: layered card shadows + one shared overlay shadow/radius (`overlay_shadow()`, `OVERLAY_CORNER_RADIUS` 8); the rail is 192 px with custom-painted rows (theme fills + 3 px accent bar, left-aligned text — egui's stock selection colors are unreachable, so every drawn pair is WCAG-tested); the desk canvas paints `desk_bg` felt, warmer than the surrounding `chrome_bg`; the status line is 28 px with group separators and a right-aligned zoom cluster [−][1:1][+][Fit] plus an ALWAYS-present % readout (a vanishing readout shifted the buttons mid-click; "1:1" kills the 100%-vs-100% confusion); zoom buttons disable on non-zoomable surfaces. Interactions: drag release renders the card at the drop position on the release frame (`release_render` — the Move applies after the frame, so without it the card flashed at its old position once); trackpad pinch (`egui::Event::Zoom`) composes multiplicatively with Ctrl+scroll (1.0015^delta) into ONE anchored zoom per frame, clamped once.
    **Equivalence-rule verification (spec §8, checked affordance-by-affordance):** every map affordance has a canonical non-map path — clusters ≈ the Drawer's tag/link chip browsing; orphans ≈ the Drawer's **Unlinked** chip; find-on-map (Ctrl+K dim) ≈ the palette, and the dim-highlight's information ≈ the palette's own result list; open (Enter) and take-to-desk (Ctrl+D) are the same shared paths on every surface. Nothing is learnable or doable only from the map.
